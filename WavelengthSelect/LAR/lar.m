...
%   from zero active variables to the ordinary least squares solution普通最小二乘解 when
%   n > p or the point where n - 1 variables are active有效 for problems where
%   n <= p.
%   BETA = LAR(X, Y, STOP) with nonzero STOP will estimate LAR solutions
%   with early stopping提早中断. If 【STOP】 is negative, STOP is an integer that
...
%   BETA = LAR(X, Y, STOP, STOREPATH) with STOREPATH set to true【真】 will
%   return the entire path from zero active variables to the point where
%   the STOP criterion停止准则 is met, or the complete path完整/全路径 for STOP = 0. Setting
%   STOREPATH to false【假】 will yield the single solution at which the STOP
%   criterion is met, thus saving precious computer resources.
%   BETA = LAR(X, Y, STOP, STOREPATH, VERBOSE) with VERBOSE冗长的 set to true
...
%   containts the goodness-of-fit estimates拟合优度估计 AIC (Akaike's Information
%   Criterion), BIC (Bayesian Information Criterion) and Mallow's Cp statistic统计值：这三个都是多元线性回归诊断指标，越小表示越精确.
%   It also includes a variable 【s】 defined as the L1 size of beta(beta的L1范数大小)
...
%   includes the number of degrees of freedom自由度 at each step and the total
